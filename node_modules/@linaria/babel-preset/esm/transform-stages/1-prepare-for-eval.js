...
export function prepareForEvalSync(babel, cache, resolve, resolvedFile, options, stack = []) {
  ...
  imports?.forEach((importsOnly, importedFile) => {
    try {
      ...
      cache.resolveCache.set(`${name} -> ${importedFile}`, `${resolved}\0${importsOnly.join(',')}`);
 ...
 */
export default async function prepareForEval(babel, cache, resolve, file, options, stack = []) {
  ...
  imports?.forEach((importsOnly, importedFile) => {
    ...
    promises.push(prepareForEval(babel, cache, resolve, promise, options, [name, ...stack]));
  });
